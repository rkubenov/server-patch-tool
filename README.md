...
> [!WARNING]
> **Обкатка неполная.** Сканирование, установка и перезагрузка проверены на одном тестовом сервере и отработали. Наблюдение за возвращением сервера дважды срывалось на живом сервере: причина найдена по журналу и устранена (монитор запускался из замыкания и получал пустой скрипт-блок), но исправление на сервере ещё не перепроверялось. Импорт из Active Directory содержал тот же дефект и, судя по коду, не работал вовсе — тоже исправлен, но не проверялся. Отложенные перепроверки долгих установок и настраиваемое время наблюдения проверены только тестами. Прежде чем применять к рабочим серверам, прогоните полный цикл на тестовом.
